...
interface LandingPageData {
  ...
  sections: Array<{
    ...
    tableHeaders?: string[];
    tableData?: string[][];
    infobox?: { text: string; style?: 'tip' | 'info'; title?: string };
    blockquote?: { text: string; source?: string };
  }>;
  imageSections?: Array<{
    src: string;
    alt: string;
...
const LandingPage = ({ data }: LandingPageProps) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-yellow-50 overflow-x-hidden">
      {/* Header */}
        ...
        <div className="container mx-auto px-4 py-4 flex justify-between items-center">
          ...
          <div className="flex items-center space-x-2">
            <Button 
              ...
              onClick={() => window.open('https://app.sonnenhelden24.de', '_blank')}
            >
              Solar-Rechner starten
            </Button>
            <Button 
              className="hidden sm:flex bg-yellow-500 hover:bg-yellow-600 text-white px-6 py-2 rounded-full transition-all duration-300 hover:scale-105"
              onClick={() => window.open('https://app.sonnenhelden24.de', '_blank')}
            >
          ...
          <div className="grid lg:grid-cols-2 gap-8 sm:gap-12 items-center">
            <div className="space-y-6 sm:space-y-8 animate-fade-in">
              <div className="space-y-3 sm:space-y-4">
                <div className="flex items-center space-x-2 text-yellow-600 mb-3 sm:mb-4">
                ...
                <p className="text-lg sm:text-xl text-gray-600 leading-relaxed">
                  <ReactMarkdown
                    components={{
                      ...
                      a: ({node, ...props}) => <a className="text-yellow-600 underline" target="_blank" rel="noopener noreferrer" {...props} />
                    }}
                  >
                    {data.heroText}
                  </ReactMarkdown>
              ...
              <div className={`grid lg:grid-cols-2 gap-8 sm:gap-12 items-center ${index % 2 === 1 ? 'lg:grid-flow-col-dense' : ''}`}>
                <div className={`space-y-4 sm:space-y-6 ${index % 2 === 1 ? 'lg:col-start-2' : ''}`}>
                  ...
                  <div className="flex space-x-4">
                    <Button 
                      className="bg-yellow-500 hover:bg-yellow-600 text-white transition-all duration-300 hover:scale-105 text-sm sm:text-base"
                      onClick={handleCTAClick}
                    >
                      Mehr erfahren
                ...
                <div className={`${index % 2 === 1 ? 'lg:col-start-1 lg:row-start-1' : ''}`}>
                  <div className="relative overflow-hidden rounded-2xl">
                    <div className="absolute inset-0 bg-gradient-to-r from-yellow-200 to-orange-200 rounded-2xl transform rotate-3 opacity-30"></div>
                    <img 
      ...
      <section className="py-12 sm:py-16 bg-yellow-500">
        <div className="container mx-auto px-4 text-center">
          <div className="grid md:grid-cols-3 gap-6 sm:gap-8 text-white">
            <div className="space-y-3 sm:space-y-4">
              <Calendar className="w-10 sm:w-12 h-10 sm:h-12 mx-auto text-yellow-200" />
              <h3 className="text-xl sm:text-2xl font-bold">Über 10 Jahre</h3>
              <p className="text-yellow-100 text-sm sm:text-base">Erfahrung in der Solartechnik</p>
            </div>
            <div className="space-y-3 sm:space-y-4">
              <Shield className="w-10 sm:w-12 h-10 sm:h-12 mx-auto text-yellow-200" />
              <h3 className="text-xl sm:text-2xl font-bold">100% Garantie</h3>
              <p className="text-yellow-100 text-sm sm:text-base">5 Jahre Gewährleistung auf alle Arbeiten</p>
            </div>
            <div className="space-y-3 sm:space-y-4">
              <CheckCircle className="w-10 sm:w-12 h-10 sm:h-12 mx-auto text-yellow-200" />
              <h3 className="text-xl sm:text-2xl font-bold">2.500+ Kunden</h3>
              <p className="text-yellow-100 text-sm sm:text-base">Zufriedene Kunden deutschlandweit</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-12 sm:py-16 bg-gradient-to-r from-orange-500 to-orange-600 overflow-hidden">
        <div className="container mx-auto px-4 text-center">
          ...
          <Button 
            size="lg" 
            className="bg-white text-yellow-700 hover:bg-gray-100 px-6 sm:px-8 py-3 sm:py-4 rounded-full text-base sm:text-lg font-bold transition-all duration-300 hover:scale-105 hover:shadow-xl w-full max-w-xs sm:max-w-sm mx-auto"
            onClick={handleCTAClick}
          >
            Jetzt Beratung anfragen
          </Button>
        ...
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8 mb-6 sm:mb-8">
            <div className="sm:col-span-2 lg:col-span-1">
              ...
              />
              <p className="text-gray-400 text-sm sm:text-base">Dein Partner für unabhängige Solarenergie in ganz Deutschland.</p>
            </div>
            <div>
